...
\[
\mathcal B_k=\{j\in\mathcal J:A_{kj}>0\}.
\]
...
Let \(x_j\ge0\) indicate the instantaneous allocation to activity \(j\) at a decision epoch. Usually \(0\le x_j\le1\). A discrete simulator may restrict actions to binary extreme points with \(x_j\in\{0,1\}\). A fractional-sharing model may instead use continuous allocation vectors.
...
\[
\kappa_k\in\{0,1\},\qquad \iota_k\in\{0,1\}.
\]
...
\[
\mathcal K_{\mathrm{proc}}=\{k\in\mathcal K:\kappa_k=0\},\qquad \mathcal K_{\mathrm{input}}=\{k\in\mathcal K:\kappa_k=1\}.
\]
...
\[
\begin{aligned}
\varrho^*=\min_{\varrho,x}\quad & \varrho\\
...
| Result | Meaning |
|---|---|
| \(\varrho^*<1\) | some static allocation keeps every physical station strictly below capacity |
| \(\varrho^*=1\) | the physical processing network is critically loaded |
| \(\varrho^*>1\) | the specified input load cannot be stabilized under the constraints |
...
|---|---|---|---|
| \(\mathcal I\) | \(\{1,2,3\}\) | \(\{1,2,3\}\) | \(\{1,\ldots,8\}\) |
...
| Resource rows encoded by \(A\) | \(\mathcal B_{u_1}=\{a_1\}\)<br>\(\mathcal B_{u_2}=\{a_2\}\)<br>\(\mathcal B_{S_1}=\{s_1,s_2\}\)<br>\(\mathcal B_{S_2}=\{s_3\}\) | \(\mathcal B_{S_1}=\{s_1,s_5\}\)<br>\(\mathcal B_{S_2}=\{s_2,s_3\}\)<br>\(\mathcal B_{S_3}=\{s_4\}\)<br>\(\mathcal B_{u_i}=\{a_i\}\) | \(\mathcal B_{S_1}=\{s_1,s_7\}\)<br>\(\mathcal B_{S_2}=\{s_2,s_5\}\)<br>\(\mathcal B_{S_3}=\{s_3,s_4,s_6,s_8\}\)<br>\(\mathcal B_{u_A}=\{a_9,a_{10}\}\)<br>\(\mathcal B_{u_B}=\{a_{11}\}\)<br>\(\mathcal B_{u_C}=\{a_{12}\}\) |
...
| \(\mu^*\in\mathbb R_+^J\) | critical primitive activity-rate vector |
...
The critical pair \((\mu^*,\beta)\) satisfies flow balance

\[
P\,\mathrm{diag}(\mu^*)\beta=0.
\]
...
\[
\mu^{(n)}=\mu^*+\frac{\hat\mu}{\sqrt n}+o(n^{-1/2}).
\]
...
After centering at \((\mu^*,\beta)\), the scaled queue satisfies
...
The sign convention matches the definition \(Y^n=(n\beta t-T^n(nt))/\sqrt n\): positive \(Y_j^n\) means activity \(j\) is reduced relative to the nominal plan. This is why \(R=-P\,\mathrm{diag}(\mu^*)\).
...
\[
\Gamma=P\,\mathrm{diag}(\mu^*\circ\beta)\,P^\top.
\]
...
Let \(\mathcal K_0=\{k\in\mathcal K_{\mathrm{input}}:\iota_k=0\}\) be the no-rejection input rows, and let \(A_0\) be the corresponding submatrix of \(A\). These rows are equality constraints:
...
| Critical rates \(\mu^*\) | \((1,1,2,2,1)\) | \((1,2,2,1,1,2,1,1)\) | \((1/2,1/3,1,1,1,1,1/2,1,1/2,1/2,1/4,1/4)\) |
| Scale-\(n\) rates \(\mu^{(n)}\) | \((1,1-n^{-1/2},2,2,1)\) | \((1,2,2,1,1,2-n^{-1/2},1-n^{-1/2},1-n^{-1/2})\) | \(\mu^*\) |
| Rates at \(n=400\) | \((1,0.95,2,2,1)\) | \((1,2,2,1,1,1.95,0.95,0.95)\) | \(\mu^*\) |
| Nominal allocation \(\beta\) | \((1,1,1/2,1/2,1)\) | \((1,1/2,1/2,1,0,1,1,1)\) | \((1/2,3/4,1/4,1/4,1/4,1/4,1/2,1/4,1/2,1/2,1,1)\) |
| Control-direction matrix \(R\) | \(-P\,\mathrm{diag}(\mu^*)\) | \(-P\,\mathrm{diag}(\mu^*)\) | \(-P\,\mathrm{diag}(\mu^*)\) |
| Nonbasic activities | none | \(s_5\) | none |
| No-rejection input equality \(A_0Y=0\) | \(A_{\{u_1,u_2\},\cdot}Y=0\) | \(A_{\{u_1,u_2,u_3\},\cdot}Y=0\) | none |
| Monotonicity matrix \(\mathsf K\) | \(A_{\{S_1,S_2\},\cdot}\) | \(A_{\{S_1,S_2,S_3\},\cdot}\) with one appended row \(-e_{s_5}^\top\) | \(A\) |
...
\[
L_i(0)=0,\qquad L_i \text{ is nondecreasing},\qquad \int_0^\infty 1_{\{Z_i(t)>0\}}\,dL_i(t)=0.
\]

The matrix \(Q\) is not learned. It is a fixed boundary device. Column \(Q_{\cdot i}\) specifies the activity-space deviation used when the diffusion is pushed away from face \(\{z_i=0\}\).
...
\[
\mathcal X(z)=\{x\in[0,1]^J:Ax\le e,\quad A_0x=A_0\beta,\quad (Cx)_i=0\ \text{for every }i\text{ with }z_i=0\}.
\]
...
\[
V^u(z)=\mathbb E_z\left[\int_0^\infty e^{-\gamma t}\{\tilde h\cdot Z^u(t)+\tilde c\cdot \mathsf K\theta_u(Z^u(t))\}\,dt+\int_0^\infty e^{-\gamma t}\chi\cdot dL(t)\right].
\]
...
\[
\mathcal L f(z)=\frac12\mathrm{tr}\{\Gamma D^2f(z)\}.
\]
...
+\min_{x\in\mathcal X(z)}\left\{\nabla V(z)\cdot R\theta_x+\tilde c\cdot\mathsf K\theta_x\right\}
=\gamma V(z).
\end{aligned}
\]

At boundary points, \(\mathcal X(z)\) removes allocations from empty buffers, while \(HL\) reflects Brownian noise back into the orthant. The boundary condition on face \(\{z_i=0\}\) is
...
\[
\pi_j(z;g)=b\{g\cdot R_{\cdot j}+(\mathsf K^\top\tilde c)_j\},\qquad j\in\mathcal J.
\]
...
\[
x^*(z)\in\arg\max_{x\in\mathcal X(z)}\sum_{j\in\mathcal J}\pi_j(z)x_j.
\]
...
\[
\mathcal M_{ki}=\{\ell:A_{k\ell}>0,\ C_{i\ell}=0,\ C_{\cdot\ell}\ne0\}.
\]
...
\[
\mathcal B_k(z)=\{j:A_{kj}>0,\ C_{ij}=0\ \text{for every }i\text{ with }z_i=0\}.
\]

For each row \(k\), choose

\[
j_k^*(z)\in\arg\max_{j\in\mathcal B_k(z)}\pi_j(z),
\]
...
x_j^*(z)=
\begin{cases}
1, & j=j_k^*(z)\text{ for a row }k\text{ and }(\iota_k=0\text{ or }\pi_j(z)>0),\\
0, & \text{otherwise}.
\end{cases}
\]
...
| policy blocks | \(\mathcal B_{S_1}(z)=\{s_1:z_1>0\}\cup\{s_2:z_2>0\}\), \(\mathcal B_{S_2}(z)=\{s_3:z_3>0\}\) |
| boundary cost | \(\chi=0\) |
...
| policy blocks | \(\mathcal B_{S_1}(z)=\{s_1:z_1>0\}\cup\{s_5:z_3>0\}\), \(\mathcal B_{S_2}(z)=\{s_2:z_1>0\}\cup\{s_3:z_2>0\}\), \(\mathcal B_{S_3}(z)=\{s_4:z_3>0\}\) |
| boundary cost | \(\chi=0\) |
...
| policy blocks | \(\mathcal B_{S_1}(z)=\{s_1:z_1>0\}\cup\{s_7:z_7>0\}\), \(\mathcal B_{S_2}(z)=\{s_2:z_2>0\}\cup\{s_5:z_5>0\}\), \(\mathcal B_{S_3}(z)=\{s_3:z_3>0,s_4:z_4>0,s_6:z_6>0,s_8:z_8>0\}\), \(\mathcal B_{u_A}(z)=\{a_9,a_{10}\}\), \(\mathcal B_{u_B}(z)=\{a_{11}\}\), \(\mathcal B_{u_C}(z)=\{a_{12}\}\) |
| boundary cost | \(\chi=0\) |
...
The BSDE solver trains \(V\) and \(\nabla V\) from simulated paths of a fixed reference diffusion. The reference policy is used only for sampling; the HJB Hamiltonian still computes the optimizing allocation \(x_g^*(z)\).
...
\[
x_g^*(z)\in\arg\max_{x\in\mathcal X(z)}\sum_{j\in\mathcal J}\pi_j(z;g)x_j.
\]

Set

\[
\theta_g^*(z)=b(\beta-x_g^*(z)).
\]

The Hamiltonian driver is

\[
F(z,g)=\tilde h\cdot z+g\cdot R\theta_g^*(z)+\tilde c\cdot\mathsf K\theta_g^*(z).
\]
...
&+\int_0^T e^{-\gamma t}\{F(\bar Z(t),G_\phi(\bar Z(t)))-G_\phi(\bar Z(t))\cdot R\bar\theta(\bar Z(t))\}\,dt\\
&+\int_0^T e^{-\gamma t}\chi\cdot d\bar L(t).
\end{aligned}
\]
...
\[
\pi_j(q)=b\{g\cdot R_{\cdot j}+(\mathsf K^\top\tilde c)_j\},\qquad j\in\mathcal J.
\]
...
\[
\mathcal A(q)=\{a\in\{0,1\}^J:Aa\le e,\quad Ca\le q,\quad A_0a=A_0\beta\}.
\]

| Constraint | Meaning |
|---|---|
| \(a\in\{0,1\}^J\) | each executable activity is either selected or not selected |
...
If the simulator allows fractional effort over a decision interval, replace \(\{0,1\}^J\) by the appropriate continuous action set. If the model has explicit rejection actions rather than input idleness, include those actions in \(\mathcal A(q)\) and charge them using the scale-\(n\) rejected-job costs from Section 2.3.
...
\[
a^*(q)\in\arg\max_{a\in\mathcal A(q)}\sum_{j\in\mathcal J}\pi_j(q)a_j.
\]
...
\[
\mathcal B_k(q)=\{j:A_{kj}>0,\ C_{\cdot j}\le q\}.
\]

For each optimized row \(k\), choose

\[
j_k^*(q)\in\arg\max_{j\in\mathcal B_k(q)}\pi_j(q).
\]
...
a_j^*(q)=
\begin{cases}
1, & j=j_k^*(q)\text{ for a row }k\text{ and }(\iota_k=0\text{ or }\pi_j(q)>0),\\
0, & \text{otherwise}.
\end{cases}
\]
...
For the Three-Station routing input row \(u_A\), the block \(\mathcal B_{u_A}(q)=\{a_9,a_{10}\}\) selects the larger of the two routing scores when at least one is positive; otherwise type-A input is rejected. The \(u_B\) and \(u_C\) rows are single-admission blocks and use the same zero-threshold rule.
